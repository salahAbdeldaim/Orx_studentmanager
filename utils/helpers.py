...
def send_telegrem_messege(student_data, message, to_value="ولي الأمر", page=None): 
    ...
    if not chat_id or not str(chat_id).strip():
        error_msg = f" لم يتم العثور على حساب تيليجرام مرتبط بـ {recipient_type}. يرجى التأكد من تفعيل الحساب أولاً."
        print(f"❌ {error_msg}")
        if page:
...
def send_telegrem_video(student_data, video_path, caption="", to_value="ولي الأمر", page=None):
    ...
    if not chat_id or not str(chat_id).strip():
        error_msg = f" لم يتم العثور على حساب تيليجرام مرتبط بـ {recipient_type}."
        print(f" {error_msg}")
        if page:
            show_error_dialog(page, error_msg)
        return False, error_msg
